...
import { deletePreset, readPresets, upsertPreset } from "../presets.js";
import { customizeFlags } from "./customize.js";
import type { Preset } from "../types.js";

export const managePresets = async (): Promise<void> => {
  while (true) {
    const action = await select<"list" | "edit" | "duplicate" | "delete" | "exit">({
    ...
    if (action === "list") {
      ...
      for (const p of presets) {
        console.log(chalk.bold(p.name) + (p.description ? chalk.dim(` — ${p.description}`) : ""));
